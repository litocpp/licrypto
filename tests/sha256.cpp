...
TEST(Sha256, IncrementalMatchesOneShot) {
  constexpr auto input =
      "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"_str;
  auto expected = licrypto::sha256_hex(input);
  ...
  for (auto chunk_size : chunk_sizes) {
    ...
    EXPECT_EQ(licrypto::sha256_hex(rstd::move(state).finalize()), expected);
  }
}
...
TEST(Sha256, IncrementalCoversPaddingBoundaries) {
  ...
  for (auto length : lengths) {
    ...
    EXPECT_EQ(licrypto::sha256_hex(rstd::move(state).finalize()),
              licrypto::sha256_hex(input.as_slice()));
  }
}
...
TEST(Sha256Digest, ParsesAndFormatsCanonicalHex) {
  ...
  auto parsed = licrypto::Sha256Digest::parse_hex(upper);
  auto lower_parsed = licrypto::Sha256Digest::parse_hex(lower);
  auto trait_parsed = rstd::from_str<licrypto::Sha256Digest>(lower);
  ...
  EXPECT_TRUE(licrypto::Sha256Digest::parse_hex("abc"_str).is_err());
  EXPECT_TRUE(
...
TEST(Sha256Digest, SupportsOneShotAndIncrementalHashing) {
  auto one_shot = licrypto::sha256_digest("abc"_str);
  auto state = licrypto::Sha256::make();
